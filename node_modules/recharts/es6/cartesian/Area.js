...
import React, { PureComponent, useCallback, useMemo, useRef, useState } from 'react';
...
function VerticalRect(_ref3) {
  ...
  var maxX = max(points.map(entry => entry.x || 0));
...
function HorizontalRect(_ref4) {
  ...
  var maxY = max(points.map(entry => entry.y || 0));
  if (isNumber(baseLine)) {
...
function AreaImpl(props) {
  ...
  var {
    needClip
  } = useNeedsClip(props.xAxisId, props.yAxisId);
  var {
  ...
  var {
    ...
    isRange,
    baseLine
  } = (_useAppSelector = useAppSelector(state => selectArea(state, props.xAxisId, props.yAxisId, isPanorama, areaSettings))) !== null && _useAppSelector !== void 0 ? _useAppSelector : {};
  var {
    height,
    width,
    left,
